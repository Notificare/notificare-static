...
//		});
		...
		this.element.down('.tag-select-field').down('select').hide();
		this.element.down('.segment-select-field').down('select').hide();
...
//				}
	...
	delegates: {
		'keyup': {
			...
			'input[name="tag-search"]': function(e){
				e.stop();
				e.stopPropagation();
				var query = e.findElement().getValue();
				if(query && query.length > 1){
					this.searchTags(query,this.application,this.onSearchTagsSuccess.bind(this),this.onSearchTagsFailure.bind(this));					
				}
			},
			'input[name="segment-search"]': function(e){
				e.stop();
				e.stopPropagation();
				var query = e.findElement().getValue();
				if(query && query.length > 1){
					this.searchSegments(query,this.application,this.onSearchSegmentsSuccess.bind(this),this.onSearchSegmentsFailure.bind(this));					
				}
			}
		...
		'click': {
			...
			'.tag-search-results a': function(e){
				e.stop();		
				this.addTag(e.findElement().up().id);
				e.findElement().up().remove();
			},
			'.delete-tag': function(e){
				e.stop();
				this.removeTag(e.findElement().up().id);
			},
			'.segment-search-results a': function(e){
				e.stop();		
				this.addSegment(e.findElement().up().id,e.findElement().innerHTML);
				e.findElement().up().remove();
			},
			'.delete-segment': function(e){
				e.stop();
				this.removeSegment(e.findElement().up().id);
			},
			'.go-next': function(e){
//				e.stopPropagation();
//				e.cancelBubble = true;
...
//				console.log(e.findElement(),this.element.down('.users'));
				...
				tags = this.element.down('.tags'),
				segments = this.element.down('.segments'),
...
//				console.log(target.hasClassName('user'),target.hasClassName('device'));
				...
				}else if (target.hasClassName('p-tags')){
					tags.show();
					this.element['audience-type'].value="tags";
					li.addClassName('active');
				}else if (target.hasClassName('p-segments')){
					segments.show();
					this.element['audience-type'].value="segments";
					li.addClassName('active');
		...
		'change': {
			'select[name="audience-type"]': function(e){
				...
				segments = select.up('.field').next(1),
				tags = select.up('.field').next(2),
				location = select.up('.field').next(3);
...
//		infoWindow.open(this.map, this.markers[this.activeMarkerIndex].marker);
	 ...
	 * 
	 */
	searchTags: function(query,application,success,failure){
		if(query && application){
			
			this.element.down('.tag-search-results').update('');
			
			var loader = new Element('div',{'class':'search-loader'});
			
			this.element.down('.tag-search-results').show();
			this.element.down('.tag-search-results').insert(loader);
			
			new Ajax.Request('/api/searchtags', {
				method: 'GET',
				parameters:{
					'q': query,
					'applicationID': application
	...
	onSearchTagsSuccess: function(response){
		var tags =  response.responseJSON;	
		
		var recipients = [];
		
		recipients = this.element.down('.tag-select-field').down('select').childElements();

		if(tags.length == 0){
			this.element.down('.tag-search-results').update('<div class="no-results">' + this.dictionary.getText('no_tags_found') + '</div>');
		}else{
			this.element.down('.tag-search-results').update('');
			tags.each(function(tag,i){
				...
				recipients.each(function(recipient){
					...
					if (recipient.value == tag) {
						isListed = true;
					}
				}.bind(this));
			
				if(!isListed){
					this.element.down('.tag-search-results').insert('<ul></ul>');
					this.element.down('.tag-search-results').down().insert({top: '<li id="tag_'+ tag +'"><a href="">' + tag + '</a></li>'});
				}
	...
	onSearchTagsFailure: function(data){
		this.element.down('.tag-search-results').update('');	
	},
	addTag: function(id){
		var tag  = id.replace('tag_','');
		this.element.down('.tag-search-results').insert('<ul></ul>');
		this.element.down('.tag-list').down().insert({top: '<li id="'+ tag +'"><span>' + tag + '</span> <a href="" class="delete-tag">'+ this.dictionary.getText('x') +'</a></li>'});
		
		this.element.down('.tag-search-results').hide();
		this.element.down('.tag-search-results').update('');
		$('tagsearch').focus();
		$('tagsearch').value='';
		$('tagsearch').setAttribute('placeholder','');
		
		this.setTag(tag);
	},
	removeTag: function(id){
		$(id).remove();
		this.element.down('.tag-select-field').down('select').childElements().each(function(tag){
			
			var t  = id.replace('tag_','');
		...
		if(this.element.down('.tag-list').down().childElements().length>1){
			$('tagsearch').setAttribute('placeholder','');
		}else{
			
			$('tagsearch').setAttribute('placeholder',this.dictionary.getText('type_a_tag'));
		}
		
	},
	setTag: function(id){
		var tag  = id.replace('tag_','');
		this.element.down('.tag-select-field').down('select').insert('<option value="'+ tag +'">'+ tag +'</option>');
		this.element.down('.tag-select-field').down('select').childElements().each(function(t){
			t.selected = true;
		}.bind(this));	
	},
	/**
	 * 
	 */
	searchSegments: function(query,application,success,failure){
		if(query && application){
			
			this.element.down('.segment-search-results').update('');
			
			var loader = new Element('div',{'class':'search-loader'});
			
			this.element.down('.segment-search-results').show();
			this.element.down('.segment-search-results').insert(loader);
			
			new Ajax.Request('/api/searchsegments', {
				method: 'GET',
				parameters:{
					'q': query,
					'applicationID': application
	...
	onSearchSegmentsSuccess: function(response){
		var segments =  response.responseJSON;	
		
		var recipients = [];
		
		recipients = this.element.down('.segment-select-field').down('select').childElements();

		if(segments.length == 0){
			this.element.down('.segment-search-results').update('<div class="no-results">' + this.dictionary.getText('no_segments_found') + '</div>');
		}else{
			this.element.down('.segment-search-results').update('');
			
			segments.each(function(segment,i){
				
				var isListed = false;

				recipients.each(function(recipient){

					if (recipient.value == segment.name) {
						isListed = true;
					}
				}.bind(this));
			
				if(!isListed){
					this.element.down('.segment-search-results').insert('<ul></ul>');
					this.element.down('.segment-search-results').down().insert({top: '<li id="segment_'+ segment._id +'"><a href="">' + segment.name + '</a></li>'});
				}
	...
	onSearchSegmentsFailure: function(data){
		this.element.down('.segment-search-results').update('');	
	},
	addSegment: function(id,name){
		var segment  = id.replace('segment_','');
		this.element.down('.segment-search-results').insert('<ul></ul>');
		this.element.down('.segment-list').down().insert({top: '<li id="'+ segment +'"><span>' + name + '</span> <a href="" class="delete-segment">'+ this.dictionary.getText('x') +'</a></li>'});
		
		this.element.down('.segment-search-results').hide();
		this.element.down('.segment-search-results').update('');
		$('segmentsearch').focus();
		$('segmentsearch').value='';
		$('segmentsearch').setAttribute('placeholder','');
		
		this.setSegment(segment);
	},
	removeSegment: function(id){
		$(id).remove();
		this.element.down('.segment-select-field').down('select').childElements().each(function(segment){
			
			var s  = id.replace('segment_','');
		...
		if(this.element.down('.segment-list').down().childElements().length>1){
			$('segmentsearch').setAttribute('placeholder','');
		}else{
			
			$('segmentsearch').setAttribute('placeholder',this.dictionary.getText('type_a_segment'));
		}
		
	},
	setSegment: function(id){
		var segment  = id.replace('segment_','');
		this.element.down('.segment-select-field').down('select').insert('<option value="'+ segment +'">'+ segment +'</option>');
		this.element.down('.segment-select-field').down('select').childElements().each(function(s){
			s.selected = true;
		}.bind(this));	
	},
	/**
	...
	resetAudience: function(){
		new Ajax.Request('/api/resetaudience', {
			method: 'GET',
			onSuccess: null,
			onFailure: null
		});
