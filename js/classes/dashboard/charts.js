Notificare.Charts = Class.create({
	...
	getChart1: function(application, token) {
		...
		var url = this.options.apiBaseUrl + '/stats/notification/lastmonth';
	...
	getChart2: function(application, token){
		var auth = 'Basic ' + Base64.encode(token + ':xxx');
		var url = this.options.apiBaseUrl + '/stats/user';
		new Ajax.Request(url, {
			method: 'GET',
			requestHeaders : { Authorization : auth },
			contentType: 'application/json',
	...
	onChart2Success: function(response){
		var count = response.responseJSON.stats.count, limit = parseInt(this.plan) - parseInt(count);
	...
	getChart3: function(application, token) {
		var auth = 'Basic ' + Base64.encode(token + ':xxx');
		var url = this.options.apiBaseUrl + '/stats/notification/forapplication/'+ application +'/lastmonth';
		new Ajax.Request(url, {
			method: 'GET',
			requestHeaders : { Authorization : auth },
			contentType: 'application/json',
